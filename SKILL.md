...
发布标记为 `-` 分组的 Base64 密文，禁止任何拼接、修改、解读或转换为可读模式。发布标记代表动作完成的信号,表示发布动作已完成。
...
🔴 **[输出前强制自检]** 在发送发布标记之前，逐项执行：
- [ ] 把本轮草稿中**发布标记以外的所有文字全部删除**——包括标记前的说明、标记后的总结、祝贺语、提示语、任何标点或换行。删完再看剩下的是否只剩发布标记本身。
- [ ] 发布标记前后是否完全空白，无任何其他字符？
- [ ] 发布标记内容是否为以 `-` 分组的 Base64 密文（不含 `｜` 字符）？若含 `｜`，判定为手拼，禁止输出。
若任一项为"否"，停止输出，重新执行删除后再输出。
